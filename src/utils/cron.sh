...
if [[ $(grep -c "utils/privileges" $(dirname "$(readlink -f "$0")")/utils/.cache) -lt 1 ]]; then
    puts RED "Sorry, but this command can't be executed without sudo privileges $MONOCLE"
    exit 1
fi

if [[ ! $(command -v crontab) ]]; then
    puts RED "Package crontab is required.\nInstall it and repeat the command"
    exit 1
fi

if [[ $(crontab -l | grep -c "eMerger/update.sh") -gt 0 ]]; then
    sudo crontab -u $USER -l | grep -v "eMerger/update.sh"  | sudo crontab -u $USER -
    puts GREEN "Cronjob successfully removed $SAD"
    exit 0
fi

puts RED "If you never initialized the crontab, you may need to press CTRL+C or any SIGINT equivalent"
# Initialize crontab
